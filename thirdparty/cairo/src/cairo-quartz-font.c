					...
					cairo_font_face_t      **font_face)
{
	...
	FontName = CFStringCreateWithCString (NULL, full_name, kCFStringEncodingUTF8);
    // ctFont = CTFontCreateWithName(FontName, 16.0, NULL); // 16.0 是字号，可调整
    // if(ctFont){
	...
	if (!cgFont) {
            ...
            for (int i = (int)strlen (full_name)-1; i >= 0; i--) {
                if (full_name[i] == '-')
                    full_name[i] = ' ';
            }
            FontName = CFStringCreateWithCString (NULL, full_name, kCFStringEncodingUTF8);
