// iOS OLE/拖放实现：与 cocoa 共用相同逻辑，拖放目标通过 SConnection::EnableDragDrop
// 委托至 SUIWindow（UIDropInteraction），剪贴板通过 SClipboard 管理。
...
HRESULT WINAPI DoDragDrop(IDataObject *pDataObject,
                          ...
                          DWORD *pdwEffect)
{
    return SConnMgr::instance()->getConnection()->DoDragDrop(pDataObject, pDropSource, dwOKEffect, pdwEffect);
}
...
HRESULT WINAPI OleSetClipboard(IDataObject *pdo)
{
    ...
    pconn->getClipboard()->setDataObject(pdo);
    CloseClipboard();
    return S_OK;
}
