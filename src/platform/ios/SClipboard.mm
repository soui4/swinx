// iOS 剪贴板实现：使用 UIPasteboard 替代 NSPasteboard。
// 结构与 cocoa/SClipboard.mm 一致：SMimeData 维护内部数据缓冲，
// flush() 时写入系统 UIPasteboard，GetData() 时从系统读取。
...
public:
  ...
  Clone(__RPC__deref_out_opt IEnumFORMATETC **ppenum) override {
    return E_NOTIMPL;
  }
...
HRESULT SMimeData::QueryGetData(FORMATETC *pformatetc) {
  ...
  @autoreleasepool {
    ...
    NSString *type = SUIDataObjectProxy::getPasteboardType(cf);
    if (!type) {
      return DV_E_FORMATETC;
    }
    UIPasteboard *pasteboard = [UIPasteboard generalPasteboard];
    if ([[pasteboard pasteboardTypes] containsObject:type]) {
      return S_OK;
    }
    // CF_TEXT 也通过 string 属性检查
    if (cf == CF_TEXT && [pasteboard stringForType:UIPasteboardTypeAutomatic] != nil) {
      return S_OK;
    }
    return DV_E_FORMATETC;
  }*/
...
HRESULT SMimeData::GetData(FORMATETC *pformatetcIn, STGMEDIUM *pmedium) {
  ...
  if (pformatetcIn->cfFormat == CF_UNICODETEXT) {
    ...
    @autoreleasepool {
      UIPasteboard *pasteboard = [UIPasteboard generalPasteboard];
      // CF_TEXT 通过 string 读取
      if (pformatetcIn->cfFormat == CF_TEXT) {
        NSString *str = [pasteboard stringForType:UIPasteboardTypeAutomatic];
        if (str) {
      ...
      NSString *type = SUIDataObjectProxy::getPasteboardType(pformatetcIn->cfFormat);
      if (!type) {
        return DV_E_FORMATETC;
      }
      if ([[pasteboard pasteboardTypes] containsObject:type]) {
...
void SMimeData::flush() {
  ...
  @autoreleasepool {
    ...
    for (auto it : m_lstData) {
      NSString *type = SUIDataObjectProxy::getPasteboardType(it->fmt);
      if (!type) {
        continue;
      }
      if (it->fmt == CF_TEXT) {
        ...
        if (str) {
          [item setValue:str forKey:type];
        }
      } else {
        NSData *data = [NSData dataWithBytes:GlobalLock(it->data)
                                      length:GlobalSize(it->data)];
        GlobalUnlock(it->data);
        [item setValue:data forKey:type];
      }
    }
    [pasteboard setItems:@[item] options:@{UIPasteboardOptionLocalOnly: @YES}];
    clear();
  }
}
...
bool SClipboard::hasFormat(UINT format) {
  ...
  @autoreleasepool {
    ...
    if(format == CF_UNICODETEXT)
      format = CF_TEXT;
    if (format == CF_TEXT) {
      return [pasteboard stringForType:UIPasteboardTypeAutomatic] != nil;
    }
    NSString *type = SUIDataObjectProxy::getPasteboardType(format);
    if (!type)
      return FALSE;
    return [[pasteboard pasteboardTypes] containsObject:type];
  }*/
}
