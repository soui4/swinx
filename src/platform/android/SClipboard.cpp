...
SClipboard::SClipboard()
    : m_clipDataObject(new SAndroidClipboardDataObject())
...
{
    if (g_platformAPI.clipboard.registerClipboardFormat)
        return g_platformAPI.clipboard.registerClipboardFormat(pszName);
...
BOOL SClipboard::emptyClipboard()
{
    ...
    if (g_platformAPI.clipboard.emptyClipboard)
    {
        g_platformAPI.clipboard.emptyClipboard();
    }
...
{
    if (g_platformAPI.clipboard.hasFormat)
        return g_platformAPI.clipboard.hasFormat(format);
    
    std::lock_guard<std::recursive_mutex> lock(m_mutex);
    FORMATETC fmt = { (CLIPFORMAT)format, nullptr, DVASPECT_CONTENT, -1, TYMED_HGLOBAL };
...
BOOL SClipboard::openClipboard(HWND hWndNewOwner)
{
    ...
    if (g_platformAPI.clipboard.openClipboard)
    {
        BOOL result = g_platformAPI.clipboard.openClipboard(hWndNewOwner);
        if (!result)
            return FALSE;
    }
...
BOOL SClipboard::closeClipboard()
{
    ...
    if (g_platformAPI.clipboard.closeClipboard)
    {
        g_platformAPI.clipboard.closeClipboard();
    }
...
{
    if (g_platformAPI.clipboard.getClipboardOwner)
        return g_platformAPI.clipboard.getClipboardOwner();
...
{
    if (g_platformAPI.clipboard.getClipboardData)
    {
        HANDLE data = g_platformAPI.clipboard.getClipboardData(uFormat);
        if (data)
            return data;
    }
    ...
    if (m_clipDataObject)
    {
        HGLOBAL hData = m_clipDataObject->dataHandle((CLIPFORMAT)uFormat);
        if (hData)
            return hData;
    }
    if (!m_externalDataObject)
    ...
    FORMATETC fmt = { (CLIPFORMAT)uFormat, nullptr, DVASPECT_CONTENT, -1, TYMED_HGLOBAL };
    STGMEDIUM medium = { 0 };
    ...
    ReleaseStgMedium(&medium);
    return m_clipDataObject->dataHandle((CLIPFORMAT)uFormat);
}
...
HANDLE SClipboard::setClipboardData(UINT uFormat, HANDLE hMem)
{
    ...
    if (g_platformAPI.clipboard.setClipboardData)
    {
        HANDLE result = g_platformAPI.clipboard.setClipboardData(uFormat, hMem);
        if (!result)
        {
            GlobalFree((HGLOBAL)hMem);
            return 0;
