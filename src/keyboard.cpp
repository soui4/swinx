...
static const KeyMap KeyTbl[] = {
    ...
    XK_Escape, VK_ESCAPE, XK_Tab, VK_TAB,
    // XK_ISO_Left_Tab,            Qt::Key_Backtab,
    XK_BackSpace, VK_BACK, XK_Return, VK_RETURN, XK_Insert, VK_INSERT, XK_Delete, VK_DELETE, XK_Clear, VK_DELETE, XK_Pause, VK_PAUSE, XK_Print, VK_PRINT,
    //    0x1005FF60,                 Qt::Key_SysReq,         // hardcoded Sun SysReq
    //    0x1007ff00,                 Qt::Key_SysReq,         // hardcoded X386 SysReq

    // cursor movement

    XK_Home, VK_HOME, XK_End, VK_END, XK_Left, VK_LEFT, XK_Up, VK_UP, XK_Right, VK_RIGHT, XK_Down, VK_DOWN, XK_Prior, VK_PRIOR, XK_Next, VK_NEXT,

    // modifiers

    XK_Shift_L, VK_SHIFT, XK_Shift_R, VK_SHIFT, XK_Shift_Lock, VK_SHIFT, XK_Control_L, VK_CONTROL, XK_Control_R, VK_CONTROL,
    //    XK_Meta_L,                  Qt::Key_Meta,
    //    XK_Meta_R,                  Qt::Key_Meta,
    XK_Alt_L, VK_MENU, XK_Alt_R, VK_MENU, XK_Caps_Lock, VK_CAPITAL, XK_Num_Lock, VK_NUMLOCK, XK_Scroll_Lock, VK_SCROLL,
    ...
    XK_KP_Space, VK_SPACE, XK_KP_Tab, VK_TAB, XK_KP_Enter, VK_RETURN,
    // XK_KP_F1,                 Qt::Key_F1,
    // XK_KP_F2,                 Qt::Key_F2,
    // XK_KP_F3,                 Qt::Key_F3,
    // XK_KP_F4,                 Qt::Key_F4,
    XK_KP_Home, VK_HOME, XK_KP_Left, VK_LEFT, XK_KP_Up, VK_UP, XK_KP_Right, VK_RIGHT, XK_KP_Down, VK_DOWN, XK_KP_Prior, VK_PRIOR, XK_KP_Next, VK_NEXT, XK_KP_End, VK_END, XK_KP_Begin, VK_CLEAR, XK_KP_Insert, VK_INSERT, XK_KP_Delete, VK_DELETE, XK_KP_Equal, VK_OEM_NEC_EQUAL, XK_KP_Multiply, VK_MULTIPLY, XK_KP_Add, VK_ADD, XK_KP_Separator, VK_SEPARATOR, XK_KP_Subtract, VK_SUBTRACT, XK_KP_Decimal, VK_DECIMAL, XK_KP_Divide, VK_DIVIDE,
...
xcb_connection_t *SKeyboard::xcb_connection()
{
...
void SKeyboard::checkForLatinLayout()
{
    m_hasLatinLayout = false;
...
uint32_t SKeyboard::xkbModMask(uint16_t state)
{
    uint32_t xkb_mask = 0;
...
void SKeyboard::updateXKBStateFromState(struct xkb_state *kb_state, uint16_t state)
{
...
uint32_t SKeyboard::translateModifiers(int s) const
{
    UINT ret = 0;
...
int SKeyboard::scanCodeToVk(xcb_keysym_t keysym, UINT modifiers) const
{
    int code = 0;
    if (keysym < 128)
    {
...
uint32_t SKeyboard::onKeyEvent(bool bPress, xcb_keycode_t code, uint16_t state, xcb_timestamp_t ts)
{
    ...
    if (vk == VK_CAPITAL) {
        m_byKeyboardState[vk] |= xkb_state_led_name_is_active(xkb_state, XKB_LED_NAME_CAPS ) ? 0x01 : 0x00;
    }
    if (vk == VK_NUMLOCK) {
        m_byKeyboardState[vk] |= xkb_state_led_name_is_active(xkb_state, XKB_LED_NAME_NUM) ? 0x01 : 0x00;
    }
    if (vk == VK_SCROLL) {
        m_byKeyboardState[vk] |= xkb_state_led_name_is_active(xkb_state, XKB_LED_NAME_SCROLL) ? 0x01 : 0x00;
    }
...
void SKeyboard::updateModifiers()
{
    ...
    xcb_get_modifier_mapping_reply_t *modMapReply = xcb_get_modifier_mapping_reply(conn, modMapCookie, &error);
    if (error)
    {
        DBG_LOG("Qt: failed to get modifier mapping");
        free(error);
        return;
    }

    // for Alt and Meta L and R are the same
    static const xcb_keysym_t symbols[] = { XK_Alt_L, XK_Meta_L, XK_Mode_switch, XK_Super_L, XK_Super_R, XK_Hyper_L, XK_Hyper_R };
...
void SKeyboard::resolveMaskConflicts()
{
