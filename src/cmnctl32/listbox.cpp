...
#define SEND_NOTIFICATION(descr,code) \
    (SendMessageA( (descr)->owner, WM_COMMAND, \
     MAKEWPARAM( GetWindowLongPtr((descr->self),GWLP_ID), (code)), (LPARAM)(descr->self) ))
...
static INT LISTBOX_FindFileStrPos( LB_DESCR *descr, LPCSTR str )
{
    ...
    while (min != max)
    {
        ...
        if (*p == '[')  /* drive or directory */
        {
            ...
            {
                if (str[1] == '-') res = 1;
                else res = LISTBOX_lstrcmpi( descr->locale, str, p );
            }
        }
        else  /* filename */
        {
    ...
    if (HAS_STRINGS(descr))
    {
        ...
        else
        {
            ...
            for (i = 0, index = start; i < descr->nb_items; i++, index++)
            {
                ...
                if (!strnicmp(str, item_str, len)) return index;
                if (item_str[0] == '[')
                {
                    if (!strnicmp(str, item_str + 1, len)) return index;
                    if (item_str[1] == '-' && !strnicmp(str, item_str + 2, len)) return index;
                }
            }
        }
    }
...
static LRESULT LISTBOX_Paint( LB_DESCR *descr, HDC hdc )
{
    ...
    if (!descr->nb_items && (descr->focus_item != -1) &&
        (descr->in_focus))
    {
...
static LRESULT CALLBACK LISTBOX_WindowProc( HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
    ...
    if (!descr)
    {
        ...
        if (msg == WM_CREATE)
        {
            ...
            if (!LISTBOX_Create( hwnd )) return -1;
 ...
 //                                                 (INT16)HIWORD(lParam) );
    ...
    case WM_MOUSEMOVE:
        ...
        {
            LISTBOX_HandleMouseMove( descr, (WORD)LOWORD(lParam),
                                     (WORD)HIWORD(lParam) );
        }
        return 0;
    case WM_LBUTTONUP:
