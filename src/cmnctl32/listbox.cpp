...
#define IS_MULTISELECT(descr) ((descr)->style & (LBS_MULTIPLESEL | LBS_EXTENDEDSEL) && !((descr)->style & LBS_NOSEL))

#define SEND_NOTIFICATION(descr, code) (SendMessageA((descr)->owner, WM_COMMAND, MAKEWPARAM(GetWindowLongPtr((descr->self), GWLP_ID), (code)), (LPARAM)(descr->self)))
...
static BOOL resize_storage(LB_DESCR *descr, UINT items_size)
{
    ...
    if (items_size > descr->items_size || items_size + LB_ARRAY_GRANULARITY * 2 < descr->items_size)
    {
        ...
        {
            items = (LB_ITEMDATA *)heap_realloc(descr->u.items, items_size * get_sizeof_item(descr));
            if (!items)
            {
...
static void set_item_data(LB_DESCR *descr, UINT index, ULONG_PTR data)
{
    if (!(descr->style & LBS_NODATA))
        descr->u.items[index].data = data;
}

static char *get_item_string(const LB_DESCR *descr, UINT index)
{
    return HAS_STRINGS(descr) ? descr->u.items[index].str : NULL;
}

static void set_item_string(const LB_DESCR *descr, UINT index, char *string)
{
    if (!(descr->style & LBS_NODATA))
        descr->u.items[index].str = string;
}
...
{
    if (!(descr->style & LBS_NODATA))
        descr->u.items[index].height = height;
}
...
static INT LISTBOX_GetCurrentPageSize(const LB_DESCR *descr)
{
    ...
    {
        if ((height += get_item_height(descr, i)) > descr->height)
            break;
    }
    if (i == descr->top_item)
        return 1;
...
static INT LISTBOX_GetMaxTopIndex(const LB_DESCR *descr)
{
    ...
    if (descr->style & LBS_OWNERDRAWVARIABLE)
    {
        ...
        for (max = descr->nb_items - 1; max >= 0; max--)
            if ((page -= get_item_height(descr, max)) < 0)
                break;
...
static void LISTBOX_UpdateSize(LB_DESCR *descr)
{
    ...
    if (!(descr->style & LBS_NOINTEGRALHEIGHT) && !(descr->style & LBS_OWNERDRAWVARIABLE))
    {
        ...
        {
            TRACE("[0x%ld]: changing height %d -> %d\n", descr->self, descr->height, descr->height - remaining);
            SetWindowPos(descr->self, 0, 0, 0, rect.right - rect.left, rect.bottom - rect.top - remaining, SWP_NOZORDER | SWP_NOACTIVATE | SWP_NOMOVE);
            return;
        }
    }
    TRACE("[0x%ld]: new size = %d,%d\n", descr->self, descr->width, descr->height);
...
static LRESULT LISTBOX_GetItemRect(const LB_DESCR *descr, INT index, RECT *rect)
{
    ...
    if (descr->style & LBS_MULTICOLUMN)
    {
        INT col = (index / descr->page_size) - (descr->top_item / descr->page_size);
    ...
    return ((rect->left < descr->width) && (rect->right > 0) && (rect->top < descr->height) && (rect->bottom > 0));
}
...
static INT LISTBOX_GetItemFromPoint(const LB_DESCR *descr, INT x, INT y)
{
    ...
    if (descr->style & LBS_OWNERDRAWVARIABLE)
    {
        ...
        if (y >= 0)
        {
            ...
            {
                if ((pos += get_item_height(descr, index)) > y)
                    break;
                index++;
            }
        }
...
static void LISTBOX_PaintItem(LB_DESCR *descr, HDC hdc, const RECT *rect, INT index, UINT action, BOOL ignoreFocus)
{
    ...
    focused = !ignoreFocus && descr->focus_item == index && descr->in_focus;
    ...
    if (IS_OWNERDRAW(descr))
    {
        ...
        dis.CtlID = GetWindowLongPtr(descr->self, GWLP_ID);
        ...
        dis.rcItem = *rect;
        // TRACE("[0x%ld]: drawitem %d (%s) action=%02x state=%02x rect=%s\n",
    ...
    else
    {
        ...
        // TRACE("[0x%ld]: painting %d (%s) action=%02x rect=%s\n",
        //      descr->self, index, debugstr_w(item_str), action,
        //      wine_dbgstr_rect(rect) );
        if (!item_str)
            ExtTextOutA(hdc, rect->left + 1, rect->top, ETO_OPAQUE | ETO_CLIPPED, rect, NULL, 0, NULL);
        else if (!(descr->style & LBS_USETABSTOPS))
            ExtTextOutA(hdc, rect->left + 1, rect->top, ETO_OPAQUE | ETO_CLIPPED, rect, item_str, strlen(item_str), NULL);
        else
        {
            /* Output empty string to paint background in the full width. */
            ExtTextOutA(hdc, rect->left + 1, rect->top, ETO_OPAQUE | ETO_CLIPPED, rect, NULL, 0, NULL);
            TabbedTextOutA(hdc, rect->left + 1, rect->top, item_str, strlen(item_str), descr->nb_tabs, descr->tabs, 0);
        }
        if (selected)
        {
...
static void LISTBOX_RepaintItem(LB_DESCR *descr, INT index, UINT action)
{
    ...
    if (!(hdc = GetDCEx(descr->self, 0, DCX_CACHE)))
        return;
    if (descr->font)
        oldFont = SelectObject(hdc, descr->font);
    hbrush = (HBRUSH)SendMessageW(descr->owner, WM_CTLCOLORLISTBOX, (WPARAM)hdc, (LPARAM)descr->self);
    if (hbrush)
 ...
 *           LISTBOX_GetText
 */
static LRESULT LISTBOX_GetText(LB_DESCR *descr, INT index, void *buffer, BOOL unicode)
{
    DWORD len;
    ...
    {
        char *str = get_item_string(descr, index);
        if (unicode)
        {
            len = MultiByteToWideChar(CP_UTF8, 0, str, -1, nullptr, 0);
            if (buffer)
            {
                MultiByteToWideChar(CP_UTF8, 0, str, -1, (WCHAR *)buffer, len);
            }
        }
        else
        {
...
static inline INT LISTBOX_lstrcmpi(LCID lcid, LPCSTR str1, LPCSTR str2)
{
...
static INT LISTBOX_FindStringPos(LB_DESCR *descr, LPCSTR str, BOOL exact)
{
    ...
    if (!descr->nb_items || !(descr->style & LBS_SORT))
    ...
    while (min <= max)
    {
        index = (min + max) / 2;
        if (HAS_STRINGS(descr))
            res = LISTBOX_lstrcmpi(descr->locale, get_item_string(descr, index), str);
        else
        {
            COMPAREITEMSTRUCT cis;
            UINT id = (UINT)GetWindowLongPtr(descr->self, GWLP_ID);
            ...
            res = SendMessageA(descr->owner, WM_COMPAREITEM, id, (LPARAM)&cis);
        }
        if (!res)
            return index;
        if (res > 0)
...
static INT LISTBOX_FindFileStrPos(LB_DESCR *descr, LPCSTR str)
{
    ...
    {
        INT index = (min + max) / 2;
        LPCSTR p = get_item_string(descr, index);
        if (*p == '[') /* drive or directory */
        {
            ...
            else /* directory */
            {
                if (str[1] == '-')
                    ...
                    res = LISTBOX_lstrcmpi(descr->locale, str, p);
            }
        }
        else /* filename */
        {
...
static INT LISTBOX_FindString(LB_DESCR *descr, INT start, LPCSTR str, BOOL exact)
{
    ...
    if (HAS_STRINGS(descr))
    {
        ...
        if (exact)
        {
            for (i = 0, index = start; i < descr->nb_items; i++, index++)
            {
                if (index == descr->nb_items)
                    index = 0;
                if (!LISTBOX_lstrcmpi(descr->locale, str, get_item_string(descr, index)))
                    return index;
            }
        }
        else
        {
            ...
            for (i = 0, index = start; i < descr->nb_items; i++, index++)
            {
                ...
                if (!strnicmp(str, item_str, len))
                    return index;
                if (item_str[0] == '[')
                {
                    if (!strnicmp(str, item_str + 1, len))
                        return index;
                    if (item_str[1] == '-' && !strnicmp(str, item_str + 2, len))
                        return index;
                }
            }
        }
    ...
    else
    {
        ...
        for (i = 0, index = start; i < descr->nb_items; i++, index++)
        {
            if (index == descr->nb_items)
                index = 0;
            if (get_item_data(descr, index) == (ULONG_PTR)str)
                return index;
        }
    }
    return LB_ERR;
...
static LRESULT LISTBOX_GetSelCount(const LB_DESCR *descr)
{
    ...
    if (!(descr->style & LBS_MULTIPLESEL) || (descr->style & LBS_NOSEL))
        return LB_ERR;
    for (i = count = 0; i < descr->nb_items; i++)
        if (is_item_selected(descr, i))
            count++;
    return count;
}
...
static LRESULT LISTBOX_Paint(LB_DESCR *descr, HDC hdc)
{
    ...
    if (descr->font)
        oldFont = SelectObject(hdc, descr->font);
    hbrush = (HBRUSH)SendMessageA(descr->owner, WM_CTLCOLORLISTBOX, (WPARAM)hdc, (LPARAM)descr->self);
    if (hbrush)
        oldBrush = SelectObject(hdc, hbrush);
    if (!IsWindowEnabled(descr->self))
        SetTextColor(hdc, GetSysColor(COLOR_GRAYTEXT));

    if (!descr->nb_items && (descr->focus_item != -1) && (descr->in_focus))
    {
        /* Special case for empty listbox: paint focus rect */
        rect.bottom = rect.top + descr->item_height;
        ExtTextOutA(hdc, 0, 0, ETO_OPAQUE | ETO_CLIPPED, &rect, NULL, 0, NULL);
    ...
    if (focusRect.top != focusRect.bottom && descr->in_focus)
...
static LRESULT LISTBOX_SetColumnWidth(LB_DESCR *descr, INT column_width)
{
    ...
    TRACE("[0x%ld]: new column width = %d\n", descr->self, column_width);
...
static void LISTBOX_MakeItemVisible(LB_DESCR *descr, INT index, BOOL fully)
{
    ...
    else
    {
        if (index < descr->top_item + descr->page_size)
            return;
        if (!fully && (index == descr->top_item + descr->page_size) && (descr->height > (descr->page_size * descr->item_height)))
            return;
...
static LRESULT LISTBOX_SetSelection(LB_DESCR *descr, INT index, BOOL on, BOOL send_notify)
{
    ...
    else
    {
        ...
        if (index == oldsel)
            return LB_OKAY;
        if (oldsel != -1)
            set_item_selected_state(descr, oldsel, FALSE);
        if (index != -1)
            set_item_selected_state(descr, index, TRUE);
        descr->selected_item = index;
        if (oldsel != -1)
        ...
        if (send_notify && descr->nb_items)
            SEND_NOTIFICATION(descr, (index != -1) ? LBN_SELCHANGE : LBN_SELCANCEL);
        // else
...
static void LISTBOX_MoveCaret(LB_DESCR *descr, INT index, BOOL fully_visible)
{
    ...
    if (descr->style & LBS_EXTENDEDSEL)
    {
        ...
        {
            INT first = std::min(index, descr->anchor_item);
            INT last = std::max(index, descr->anchor_item);
            if (first > 0)
 ...
 *           LISTBOX_InsertItem
 */
static LRESULT LISTBOX_InsertItem(LB_DESCR *descr, INT index, LPSTR str, ULONG_PTR data)
{
    ...
    if (index == -1)
        ...
        return LB_ERR;
    if (!resize_storage(descr, descr->nb_items + 1))
        return LB_ERR;
    ...
    if (descr->style & LBS_OWNERDRAWVARIABLE)
    {
        ...
        // TRACE("[0x%ld]: measure item %d (%s) = %d\n",
...
static LRESULT LISTBOX_InsertString(LB_DESCR *descr, INT index, LPCSTR str)
{
    ...
    if (HAS_STRINGS(descr))
    {
        if (!str)
            str = "";
        if (!(new_str = (LPSTR)HeapAlloc(GetProcessHeap(), 0, (strlen(str) + 1) * sizeof(char))))
        {
            SEND_NOTIFICATION(descr, LBN_ERRSPACE);
            return LB_ERRSPACE;
        }
...
static LRESULT LISTBOX_HandleVScroll(LB_DESCR *descr, WORD scrollReq, WORD pos)
{
    ...
    case SB_LINEDOWN:
        ...
        break;
    case SB_PAGEUP:
        LISTBOX_SetTopItem(descr, descr->top_item - LISTBOX_GetCurrentPageSize(descr), TRUE);
        break;
    case SB_PAGEDOWN:
        LISTBOX_SetTopItem(descr, descr->top_item + LISTBOX_GetCurrentPageSize(descr), TRUE);
        break;
...
static LRESULT LISTBOX_HandleHScroll(LB_DESCR *descr, WORD scrollReq, WORD pos)
{
    ...
    if (descr->style & LBS_MULTICOLUMN)
    {
        ...
        {
        case SB_LINELEFT:
            LISTBOX_SetTopItem(descr, descr->top_item - descr->page_size, TRUE);
            break;
        case SB_LINERIGHT:
            LISTBOX_SetTopItem(descr, descr->top_item + descr->page_size, TRUE);
            break;
        case SB_PAGELEFT:
            ...
            if (page < 1)
                page = 1;
            LISTBOX_SetTopItem(descr, descr->top_item - page * descr->page_size, TRUE);
            break;
        case SB_PAGERIGHT:
            page = descr->width / descr->column_width;
            if (page < 1)
        ...
        case SB_THUMBTRACK:
            ...
            info.fMask = SIF_TRACKPOS;
            GetScrollInfo(descr->self, SB_VERT, &info);
            LISTBOX_SetTopItem(descr, info.nTrackPos * descr->page_size, TRUE);
            break;
        case SB_LEFT:
            LISTBOX_SetTopItem(descr, 0, TRUE);
            break;
    ...
    else if (descr->horz_extent)
    {
        ...
        case SB_LINERIGHT:
            ...
            break;
        case SB_PAGELEFT:
            LISTBOX_SetHorizontalPos(descr, descr->horz_pos - descr->width);
            break;
        case SB_PAGERIGHT:
            LISTBOX_SetHorizontalPos(descr, descr->horz_pos + descr->width);
            break;
        ...
        case SB_LEFT:
            ...
            break;
        case SB_RIGHT:
            LISTBOX_SetHorizontalPos(descr, descr->horz_extent - descr->width);
            break;
        }
    }
    return 0;
...
static LRESULT LISTBOX_HandleMouseWheel(LB_DESCR *descr, SHORT delta)
{
    ...
    if ((delta < 0 && descr->wheel_remain < 0) || (delta > 0 && descr->wheel_remain > 0))
    ...
    if (descr->wheel_remain && pulScrollLines)
    {
        ...
        if (descr->style & LBS_MULTICOLUMN)
        {
            pulScrollLines = std::min(descr->width / descr->column_width, pulScrollLines);
            pulScrollLines = std::max(1, pulScrollLines);
        ...
        {
            pulScrollLines = std::min(descr->page_size, pulScrollLines);
            cLineScroll = pulScrollLines * descr->wheel_remain / WHEEL_DELTA;
            descr->wheel_remain -= WHEEL_DELTA * cLineScroll / pulScrollLines;
        }
...
static LRESULT LISTBOX_HandleLButtonDown(LB_DESCR *descr, DWORD keys, INT x, INT y)
{
    ...
    // TRACE("[0x%ld]: lbuttondown %d,%d item %d, focus item %d\n",
    //      descr->self, x, y, index, descr->focus_item);

    if (!descr->in_focus)
    {
        printf("setfocus for listbox\n");
        SetFocus(descr->self);
    }
    ...
    if (descr->style & (LBS_EXTENDEDSEL | LBS_MULTIPLESEL))
    {
        ...
        */
        ...
        if (keys & MK_CONTROL)
        {
            ...
            LISTBOX_SetSelection(descr, index, !is_item_selected(descr, index), (descr->style & LBS_NOTIFY) != 0);
        }
        else
        {
    ...
    else
    {
        descr->anchor_item = index;
        LISTBOX_MoveCaret(descr, index, FALSE);
        LISTBOX_SetSelection(descr, index, TRUE, (descr->style & LBS_NOTIFY) != 0);
    }
...
static LRESULT LISTBOX_HandleLButtonUp(LB_DESCR *descr)
{
    if (LISTBOX_Timer != LB_TIMER_NONE)
        KillTimer(descr->self, LB_TIMER_ID);
    LISTBOX_Timer = LB_TIMER_NONE;
    if (descr->captured)
    {
        descr->captured = FALSE;
        if (GetCapture() == descr->self)
            ReleaseCapture();
...
static void LISTBOX_HandleMouseMove(LB_DESCR *descr, INT x, INT y)
{
    ...
    if (dir != LB_TIMER_NONE)
        SetTimer(descr->self, LB_TIMER_ID, LB_SCROLL_TIMEOUT, NULL);
...
static LRESULT LISTBOX_HandleKeyDown(LB_DESCR *descr, DWORD key)
{
    ...
    {
        caret = SendMessageW(descr->owner, WM_VKEYTOITEM, MAKEWPARAM(LOWORD(key), descr->focus_item), (LPARAM)descr->self);
        if (caret == -2)
            return 0;
    }
    if (caret == -1)
        ...
        case VK_RIGHT:
            ...
            {
                bForceSelection = FALSE;
                caret = std::min(descr->focus_item + descr->page_size, descr->nb_items - 1);
                break;
            }
            /* fall through */
        case VK_DOWN:
        ...
        case VK_SPACE:
            ...
            {
                LISTBOX_SetSelection(descr, descr->focus_item, !is_item_selected(descr, descr->focus_item), (descr->style & LBS_NOTIFY) != 0);
            }
            break;
        default:
            bForceSelection = FALSE;
    ...
    if (caret >= 0)
    {
        if (((descr->style & LBS_EXTENDEDSEL) && !(GetKeyState(VK_SHIFT) & 0x8000)) || !IS_MULTISELECT(descr))
...
static LRESULT LISTBOX_HandleChar(LB_DESCR *descr, char charW)
{
    ...
    if (descr->style & LBS_WANTKEYBOARDINPUT)
    {
        caret = SendMessageW(descr->owner, WM_CHARTOITEM, MAKEWPARAM(charW, descr->focus_item), (LPARAM)descr->self);
        if (caret == -2)
            return 0;
    }
    if (caret == -1)
...
static BOOL LISTBOX_Create(HWND hwnd /*, LPHEADCOMBO lphc*/)
{
    ...
    descr->style = GetWindowLong(descr->self, GWL_STYLE);
    descr->width = rect.right - rect.left;
    descr->height = rect.bottom - rect.top;
    descr->u.items = NULL;
    descr->items_size = 0;
    ...
    /*if( lphc )
    {
        TRACE("[0x%ld]: resetting owner %p -> %p\n", descr->self, descr->owner, lphc->self );
        descr->owner = lphc->self;
    }*/

    SetWindowLongPtr(descr->self, GWLP_OPAQUE, (LONG_PTR)descr);
    ...
    // TRACE("owner: 0x%ld, style: %08x, width: %d, height: %d\n", descr->owner, descr->style, descr->width, descr->height);
    return TRUE;
}
...
static BOOL LISTBOX_Destroy(LB_DESCR *descr)
{
    ...
    SetWindowLongPtr(descr->self, GWLP_OPAQUE, 0);
...
static LRESULT CALLBACK LISTBOX_Msg_W2A(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    LB_DESCR *descr = (LB_DESCR *)GetWindowLongPtr(hwnd, GWLP_OPAQUE);
    switch (msg)
    {
    case LB_ADDFILE:
    case LB_ADDSTRING:
    ...
    case LB_GETTEXT:
    {
        return LISTBOX_GetText(descr, wParam, (void *)lParam, TRUE);
    }

    case LB_GETTEXTLEN:
    {
        return LISTBOX_GetText(descr, wParam, nullptr, TRUE);
    }
    }
    return LISTBOX_WindowProc(hwnd, msg, wParam, lParam);
}
/***********************************************************************
...
static LRESULT CALLBACK LISTBOX_WindowProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (!descr)
    {
        ...
        if (msg == WM_CREATE)
        {
            ...
            if (!LISTBOX_Create(hwnd))
                return -1;
            // TRACE("creating hwnd 0x%ld descr %p\n", hwnd, (void *)GetWindowLongPtr( hwnd, GWLP_OPAQUE ) );
            return 0;
        }
    ...
    // TRACE("[0x%ld]: msg %#x wp %08lx lp %08lx\n", descr->self, msg, wParam, lParam );

    switch (msg)
    {
    case WM_MSG_W2A:
    {
        MSG *msgWrap = (MSG *)lParam;
        return LISTBOX_Msg_W2A(hwnd, msgWrap->message, msgWrap->wParam, msgWrap->lParam);
    }
    ...
    case LB_GETTEXT:
        return LISTBOX_GetText(descr, wParam, (LPSTR)lParam, FALSE);
    ...
    case LB_GETTEXTLEN:
        ...
        return strlen(get_item_string(descr, wParam));
    ...
    case LB_ITEMFROMPOINT:
    {
        ...
        if (!PtInRect(&rect, pt))
        {
            pt.x = std::min(pt.x, rect.right - 1);
            pt.x = std::max(pt.x, 0);
            pt.y = std::min(pt.y, rect.bottom - 1);
            pt.y = std::max(pt.y, 0);
            hit = FALSE;
        }
    ...
    case LB_SETCARETINDEX:
        if ((!IS_MULTISELECT(descr)) && (descr->selected_item != -1))
            return LB_ERR;
        return LISTBOX_SetCaretIndex(descr, wParam, !lParam);
    ...
    case LB_FINDSTRING:
        return LISTBOX_FindString(descr, wParam, (const char *)lParam, FALSE);

    case LB_FINDSTRINGEXACT:
        return LISTBOX_FindString(descr, wParam, (const char *)lParam, TRUE);
    ...
    case LB_SELITEMRANGE:
        if (LOWORD(lParam) <= HIWORD(lParam))
            return LISTBOX_SelectItemRange(descr, LOWORD(lParam), HIWORD(lParam), wParam);
        else
            return LISTBOX_SelectItemRange(descr, HIWORD(lParam), LOWORD(lParam), wParam);
    ...
    case LB_SETANCHORINDEX:
        ...
        // case LB_DIR:
        //    return LISTBOX_Directory( descr, wParam, (const char *)lParam, msg == LB_DIR );
    ...
    case WM_MOUSEWHEEL:
        if (wParam & (MK_SHIFT | MK_CONTROL))
            return DefWindowProc(descr->self, msg, wParam, lParam);
    ...
    case WM_LBUTTONDOWN:
        ...
        //                                                 (INT16)LOWORD(lParam),
        //                                                 (INT16)HIWORD(lParam) );
        return LISTBOX_HandleLButtonDown(descr, wParam, (WORD)LOWORD(lParam), (WORD)HIWORD(lParam));
    ...
    case WM_MOUSEMOVE:
        if (GetCapture() == descr->self)
        {
            LISTBOX_HandleMouseMove(descr, (WORD)LOWORD(lParam), (WORD)HIWORD(lParam));
        }
        return 0;
    case WM_LBUTTONUP:
    ...
    {
        RECT rect;
        HBRUSH hbrush = (HBRUSH)SendMessageW(descr->owner, WM_CTLCOLORLISTBOX, wParam, (LPARAM)descr->self);
        // TRACE("hbrush = %p\n", hbrush);
        if (!hbrush)
    ...
    default:
        if ((msg >= WM_USER) && (msg < 0xc000))
            WARN("[0x%ld]: unknown msg %04x wp %08lx lp %08lx\n", hwnd, msg, wParam, lParam);
    }
...
void LISTBOX_Register(void)
{
    ...
    wndClass.lpszClassName = WC_LISTBOX;
